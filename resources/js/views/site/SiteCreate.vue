                ...
                <div class="page-title-heading">
                    ...
                    <div>
                        ...
                        <div class="page-title-subheading text-muted">
                            Registra una nueva sede para este cliente
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        async submit() {
            ...
            try {
                await siteService.store(this.clientId, this.form);
                this.$router.push({ name: 'clients.show', params: { id: this.clientId } });
            } catch (err) {
                if (err.response?.status === 422) {
                    this.errors = err.response.data.errors;
                } else {
                    this.generalError = err.response?.data?.message || 'Error al crear la sede.';
                }
            } finally {
                this.loading = false;
            }
