...
class DemoSeeder extends Seeder
{
    ...
    /**
     * Mes al que se escribieron a mano las tres agendas de este seeder.
     *
     * Los datos demo se anclaron a mayo de 2026 y envejecen: en septiembre el portal
     * del cliente mostraba «0 reportes este mes» y el cumplimiento de mantenimiento en
     * rojo al 0 %, porque el ultimo preventivo tenia cuatro meses. Un demo que se ve
     * peor cada mes que pasa no sirve para ensenarlo.
     *
     * En vez de reescribir las agendas, todas las fechas se desplazan los meses
     * transcurridos desde este ancla (ver desfase()), asi el ultimo preventivo cae
     * siempre en el mes en curso. El folio sale de $date->year, de modo que se
     * mantiene coherente al moverse.
     */
    private const ANCLA = '2026-05-01';

    private ?int $desfase = null;
    ...
    /** Meses que hay que sumar a las fechas escritas a mano para que caigan en el presente. */
    private function desfase(): int
    {
        return $this->desfase ??= (int) Carbon::parse(self::ANCLA)
            ->startOfMonth()
            ->diffInMonths(now()->startOfMonth());
    }

    public function run(): void
    {
        $this->command?->info("Desfase de fechas: +{$this->desfase()} meses sobre el ancla ".self::ANCLA);
    ...
    private function cleanup(): void
    {
        ...
        //
        // Las cuatro tablas de abajo llegaron despues de este seeder y apuntan al equipo
        // con ON DELETE RESTRICT, asi que sin borrarlas primero el seeder deja de poder
        // re-ejecutarse: falla en el cleanup con un 1451 y no se entera nadie hasta que
        // se intenta refrescar el demo. Se borran por equipo, no por cliente, porque es
        // la unica columna que las cuatro comparten.
        $sites = Site::where('client_id', $client->id)->get();
        foreach ($sites as $s) {
            $equipmentIds = Equipment::where('site_id', $s->id)->pluck('id');
            if ($equipmentIds->isNotEmpty()) {
                DB::table('visit_reminders')
                    ->whereIn('scheduled_visit_id', DB::table('scheduled_visits')
                        ->whereIn('equipment_id', $equipmentIds)->pluck('id'))
                    ->delete();
                DB::table('reschedule_requests')
                    ->whereIn('scheduled_visit_id', DB::table('scheduled_visits')
                        ->whereIn('equipment_id', $equipmentIds)->pluck('id'))
                    ->delete();
                DB::table('scheduled_visits')->whereIn('equipment_id', $equipmentIds)->delete();
                DB::table('technician_checkins')->whereIn('equipment_id', $equipmentIds)->delete();
            }
    ...
    private function generateRSTP(array $equipments, Client $client, Site $site): int
    {
        ...
        $count = 0;
        $hoy = now()->startOfDay();
        // addMonths() DESBORDA: 31-may + 4 meses cae en 1-oct, no en 30-sep, y el demo
        // acababa con reportes en el futuro. El fin es siempre el mes en curso.
        $start = Carbon::create(2024, 7, 1)->addMonthsNoOverflow($this->desfase());
        $end = now()->endOfMonth();
        ...
        for ($date = $start->copy(); $date->lte($end); $date->addMonth()) {
            foreach ($equipments as $eqIdx => $eq) {
                ...
                // El mes en curso puede no haber llegado al dia 8, y entonces la ronda
                // caeria en el futuro. Se coloca en los ultimos dias ya transcurridos,
                // escalonada por equipo. Sin esto, el panel del cliente muestra 0 % de
                // cumplimiento durante la primera semana de cada mes.
                if ($serviceDate->gt($hoy)) {
                    $serviceDate = $hoy->copy()->subDays($eqIdx);
                    if ($serviceDate->lt($date)) {
                        continue;   // el mes aun no da ni para escalonar la ronda
                    }
                }
    ...
    private function generateRSTC(array $equipments, Client $client, Site $site): int
    {
        ...
        foreach ($schedule as $i => [$eqIdx, $year, $month]) {
            $eq = $equipments[$eqIdx];
            $serviceDate = Carbon::create($year, $month, 5 + ($i % 20))->addMonthsNoOverflow($this->desfase());
            if ($serviceDate->gt(now())) {
                continue;   // un correctivo con fecha futura no existe
            }
    ...
    private function generateRSTE(array $equipments, Client $client, Site $site): int
    {
        ...
        foreach ($schedule as $i => [$eqIdx, $year, $month, $description]) {
            $eq = $equipments[$eqIdx];
            $serviceDate = Carbon::create($year, $month, 12 + ($i % 10))->addMonthsNoOverflow($this->desfase());
            if ($serviceDate->gt(now())) {
                continue;
            }
            $techId = $this->technicians[$i % count($this->technicians)];
